...
pub trait GltfObject{
    ...
    fn parse_extension<T: GltfExtension>(&self) -> Result<T, ParseExtensionError>{
        let extension = self.extensions().as_ref().and_then(|extensions| extensions.get(T::extension_name()));
        if let Some(extension) = extension{
            serde_json::from_value::<T>(extension.clone()).map_err(|e| ParseExtensionError::FailedToParse{inner: Box::new(e)})
        }
        else{
